...
/*
	FPEPushField - 밀치기 판정이 바라보는 전장 (기본적으로 GridSystem 그 자체)
	...
	실제 점유는 이동이 '시작될 때' 목적지로 갱신 (UACGridMovementComponent::ProcessNextCommand),
	따라서 실행 경로는 앞 요청이 출발한 결과를 별도 처리 없이 그대로 보게 됩니다.
*/
...
/*
	FPEPushResolver - 밀치기 규칙의 단일 출처 (상태 없는 순수 함수 모음)

	월드를 바꾸지 않으므로 클라이언트에서도 안전하게 호출 가능
	연쇄를 한꺼번에 전개하지 않고 ResolveSingle 한 번에 한 명씩만 품

	실행		- 실제 도착 이벤트로 다음 ResolveSingle을 부름
	시각화	- SimulateChain으로 끝까지 펼침
*/
struct PROJECT_ENTROPY_API FPEPushResolver
{
	...
	/*
		시각화 전용: 연쇄를 끝까지 펼쳐 예상 결과 전체를 반환

		내부가 ResolveSingle + MakeChainedRequest 반복이므로 실행과 규칙이 갈라질 수 없음
		다만 아무도 죽지 않는다는 가정 위의 예측이므로, 연쇄 도중 사망이 일어나면 실제와 달라질 수 있음

		Field에 가정이 누적되므로 여러 번 나눠 호출하면 앞 호출의 결과를 이어받음
		관통 투사체처럼 서버가 대상을 한 명씩 순차로 미는 경우를 그대로 재현할 때 필요
	*/
