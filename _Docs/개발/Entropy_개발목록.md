...
현재 코드에 남아 있는 동작 불일치 3건.

### 0-1. Custom AoE 비주얼 회전 스냅 통일

**작업** — `GetAoEBoundsAndRotation`의 회전 계산을 `Atan2` 연속각에서 `GetCardinalQuarterTurns` 기반 4방향 스냅으로 변경.

**배경** — 타일 집합(`GetAffectedGridPositions`)과 밀치기(`FPEPushResolver::ResolveDirection`)는 4방향 스냅을 쓰지만, 폭발 메시·VFX 회전만 연속각을 쓴다. 대각선 조준 시 칠해지는 타일과 이펙트 방향이 어긋난다.

**대상** — `Private/CardSystem/PE_SkillData.cpp:133`

**완료 기준** — 대각선(30°, 45°, 60°)으로 조준했을 때 칠해진 타일 방향과 폭발 이펙트 방향이 일치.
